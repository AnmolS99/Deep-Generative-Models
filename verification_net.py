...
class VerificationNet:
    ...
    def __init__(self,
                 ...
                 file_name: str = "./models/verification_model") -> None:
        """
    ...
    def predict(self, data: np.ndarray) -> tuple:
        ...
        for channel in range(no_channels):
            ...
            predictions += np.argmax(channel_prediction, axis=1) * np.power(
                10, channel)

        return predictions, beliefs

    def check_class_coverage(self,
                             data: np.ndarray,
                             tolerance: np.float = .8) -> np.float:
        """
